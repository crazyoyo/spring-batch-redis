...
public abstract class AbstractFileItemWriter<T> extends AbstractItemStreamItemWriter<T>
	...
	public void close() {
		...
		if (state != null) {
			try {
				...
				state.close();
				if (state.linesWritten == 0 && shouldDeleteIfEmpty && isFileSystemResource()) {
					try {
	...
	protected class OutputState {
		...
		public long position() throws IOException {
			...
			pos = isFileSystemResource() ? ((FileChannel) fileChannel).position() : ((CountingOutputStream) os).getCount();
			if (transactional) {
		...
		public void truncate() throws IOException {
			if (fileChannel instanceof FileChannel) {
				((FileChannel) fileChannel).truncate(lastMarkedByteOffsetPosition);
				((FileChannel) fileChannel).position(lastMarkedByteOffsetPosition);
			}
		}
		...
		private void initializeBufferedWriter() throws IOException {
			if (isFileSystemResource()) {
				...
				fileChannel = ((FileOutputStream) os).getChannel();
			} else {
				...
				if (transactional) {
					...
					return new BufferedWriter(Channels.newWriter(fileChannel, encoding)) {
						...
						public void flush() throws IOException {
							super.flush();
							if (forceSync && channel instanceof FileChannel) {
		...
		private void checkFileSize() throws IOException {
			...
			outputBufferedWriter.flush();
			size = isFileSystemResource() ? ((FileChannel) fileChannel).size() : ((CountingOutputStream) os).getCount();
	...
	private boolean isFileSystemResource() {
