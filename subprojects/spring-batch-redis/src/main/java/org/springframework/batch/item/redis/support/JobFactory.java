...
public class JobFactory {
    ...
    public <I, O> JobExecutionWrapper run(String name, ItemReader<? extends I> reader, ItemProcessor<I, O> processor, ItemWriter<O> writer) throws Throwable {
    ...
    public <I, O> JobExecutionWrapper runFlushing(String name, PollableItemReader<? extends I> reader, ItemWriter<O> writer) throws Throwable {
        return runFlushing(name, reader, null, writer);
    }

    public <I, O> JobExecutionWrapper runFlushing(String name, PollableItemReader<? extends I> reader, ItemProcessor<I, O> processor, ItemWriter<O> writer) throws Throwable {
        TaskletStep step = flushing(step(name, reader, processor, writer)).build();
        JobExecutionWrapper execution = new JobExecutionWrapper(asyncLauncher.run(job(name, step).build(), new JobParameters())).awaitRunning();
        awaitOpen(reader);
        return execution;
    }

    public void awaitOpen(PollableItemReader<?> reader) throws InterruptedException, TimeoutException {
        Timer timer = new Timer(readerOpenTimeout);
        while (reader.getState() == null && timer.isActive()) {
            Thread.sleep(sleep);
        }
    ...
    public <I, O> SimpleStepBuilder<I, O> step(String name, ItemReader<? extends I> reader, ItemProcessor<I, O> processor, ItemWriter<O> writer) {
    ...
    public class JobExecutionWrapper {
        ...
        public JobExecutionWrapper awaitTermination(Duration timeout) throws Throwable {
            Timer timer = new Timer(timeout);
            while (jobExecution.isRunning() && timer.isActive()) {
                Thread.sleep(sleep);
            }
    ...
    public JobExecutionWrapper runAsync(Job job, JobParameters parameters) throws JobInstanceAlreadyCompleteException, JobExecutionAlreadyRunningException, JobParametersInvalidException, JobRestartException {
