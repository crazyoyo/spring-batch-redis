...
public class StreamItemReaderTests extends AbstractRedisTestBase {
	...
	void testStreamReader(RedisServer server) throws Exception {
		...
		RedisStreamItemReader<String, String> reader = streamReader(server,
				XReadArgs.StreamOffset.from("stream:0", "0-0")).build();
	...
	void testStreamReaderJob(RedisServer redis) throws Exception {
		...
		RedisStreamItemReader<String, String> reader = streamReader(redis,
				XReadArgs.StreamOffset.from("stream:0", "0-0")).build();
	...
	void testMultipleStreamReaders(RedisServer redis) throws Exception {
		...
		dataGenerator(redis).dataTypes(DataStructure.STREAM).end(100).build().call();
		RedisStreamItemReader<String, String> reader1 = streamReader(redis, XReadArgs.StreamOffset.from(stream, "0-0"))
				.consumerGroup(consumerGroup).consumer("consumer1").ackPolicy(RedisStreamItemReader.AckPolicy.MANUAL)
				.build();
		RedisStreamItemReader<String, String> reader2 = streamReader(redis, XReadArgs.StreamOffset.from(stream, "0-0"))
		...
		JobExecutionWrapper execution1 = jobFactory.runFlushing(name(redis, "stream-reader-1"), reader1, writer1);
		ListItemWriter<StreamMessage<String, String>> writer2 = new ListItemWriter<>();
		JobExecutionWrapper execution2 = jobFactory.runFlushing(name(redis, "stream-reader-2"), reader2, writer2);
