...
public class DataGenerator implements Callable<Long> {
	...
	public DataGenerator(Supplier<StatefulConnection<String, String>> connectionSupplier,
			Function<StatefulConnection<String, String>, RedisModulesAsyncCommands<String, String>> async, int start,
			int end, long sleep, Duration minExpire, Duration maxExpire, int batchSize, Set<String> dataTypes) {
		...
		public Long call() throws InterruptedException {
			...
			for (int index = start; index < end; index++) {
				if (contains(DataStructure.STRING)) {
					...
					futures.add(
							((RedisStringAsyncCommands<String, String>) commands).set(stringKey, stringValue(index)));
					if (!maxExpire.isZero()) {
						long time = System.currentTimeMillis() + minExpire.toMillis()
								+ random.nextInt(Math.toIntExact(maxExpire.toMillis()));
				...
				if (contains(DataStructure.ZSET)) {
					futures.add(((RedisSortedSetAsyncCommands<String, String>) commands).zadd(zsetKey(index), index % 3,
							member(index)));
				}
				if (contains(DataStructure.STREAM)) {
					futures.add(
							((RedisStreamAsyncCommands<String, String>) commands).xadd(streamKey(index), hash(index)));
				}
				if (contains(DataStructure.LIST)) {
					futures.add(
							((RedisListAsyncCommands<String, String>) commands).lpush(listKey(index), member(index)));
				}
	...
	public static DataGeneratorBuilder client(AbstractRedisClient client) {
	...
	public static class DataGeneratorBuilder extends CommandBuilder<String, String, DataGeneratorBuilder> {
		...
		private Set<String> dataTypes = new HashSet<>(Arrays.asList(DataStructure.HASH, DataStructure.LIST,
				DataStructure.STRING, DataStructure.STREAM, DataStructure.SET, DataStructure.ZSET));
		...
		public DataGenerator build() {
			return new DataGenerator(connectionSupplier(), async(), start, end, sleep, minExpire, maxExpire, batchSize,
					new HashSet<>(dataTypes));
		}
	}
