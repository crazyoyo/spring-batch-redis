...
=== `KeyValue`
A `KeyValue` represents a Redis data structure and stores a key, its TTL in seconds, type of data structure (hash, list, ...), and Java representation.
...
There are two kinds of key readers:

* Snapshot: uses SCAN command to produce keys.
* Live (AKA continuous): also uses SCAN but also keyspace notifications to keep producing keys that have been updated.
