...
class StepTests {
    ...
    void flushingFaultTolerantStep() throws Exception {
        ...
        FlushingStepBuilder<Struct<String>, Struct<String>> step = new FlushingStepBuilder<>(stepBuilderFactory.get(name));
        step.chunk(1);
        step.reader(reader);
        step.writer(writer);
        step.idleTimeout(Duration.ofMillis(300));
        FlushingFaultTolerantStepBuilder<Struct<String>, Struct<String>> ftStep = step.faultTolerant();
