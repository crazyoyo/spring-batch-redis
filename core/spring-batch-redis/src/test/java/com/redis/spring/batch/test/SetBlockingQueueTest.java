...
class SetBlockingQueueTest {
    ...
    /**
     * This test creates a thread for each major queue operation (i.e. add, offer, offer w/timeout, put, poll, poll w/timeout,
     * take, remove) and an additional thread for the size and remaining capacity operations.
     *
     * Initially, we create a queue of size maxCapacity, and sequentially add numInitialEntries elements to that queue.
     *
     * The producer operations/threads (i.e. add, offer, offer w/timeout, and put) will all add entriesPerOperation elements to
     * the queue each.
     *
     * The consumer operations/threads (i.e. poll, poll w/timeout, take, and remove) will all remove entriesPerOperation
     * elements from the queue each.
     *
     * Each of these threads is run concurrently, so each operation will need to rely on the BoundedBlockingQueue locks in order
     * to control access to the internal queue.
     *
     * If all of the threads have finished, then we should be left with numInitialEntries elements in the queue.
     */
    private void testConcurrencyInternal(final int maxCapacity, final int numInitialEntries, final int entriesPerOperation,
            final int maxTimeoutSeconds) throws Exception {
        final SetBlockingQueue<String> bbq = new SetBlockingQueue<>(new PriorityBlockingQueue<>(), maxCapacity);
    ...
    /**
     * This is a helper method which will allow us to run all of our threads simultaneously, thus increasing the chance that we
     * will encounter a concurrency problem if there is one.
     *
     * All of the runnables that are passed in will block, and will not begin execution until the afterInitBlocker
     * CountDownLatch has been activated. This should ensure that all of the threads begin execution simultaneously.
     */
