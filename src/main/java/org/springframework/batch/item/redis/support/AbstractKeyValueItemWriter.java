...
public abstract class AbstractKeyValueItemWriter<K, V, C extends StatefulConnection<K, V>, T extends KeyValue<K, ?>> extends AbstractRedisItemWriter<K, V, C, T> {
    ...
    protected void write(BaseRedisAsyncCommands<K, V> commands, List<RedisFuture<?>> futures, T item) {
        if (item.getValue() == null || item.getTtl() == KeyValue.TTL_NOT_EXISTS) {
