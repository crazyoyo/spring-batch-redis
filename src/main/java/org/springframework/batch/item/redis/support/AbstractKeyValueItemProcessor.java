...
public abstract class AbstractKeyValueItemProcessor<K, V, T> implements ItemProcessor<List<? extends K>, List<T>> {
    ...
    protected AbstractKeyValueItemProcessor(GenericObjectPool<? extends StatefulConnection<K, V>> pool, Function<StatefulConnection<K, V>, BaseRedisAsyncCommands<K, V>> commands, Duration commandTimeout) {
        this.pool = pool;
