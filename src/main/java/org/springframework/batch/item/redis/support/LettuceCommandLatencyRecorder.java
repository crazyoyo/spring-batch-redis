 ...
 */
public class LettuceCommandLatencyRecorder implements CommandLatencyRecorder {
	...
	private static final String COMPLETION_METRIC = "lettuce.command.completion";
	private static final String FIRST_RESPONSE_METRIC = "lettuce.command.firstresponse";
	...
	public LettuceCommandLatencyRecorder(MeterRegistry meterRegistry, boolean enableHistogram) {
	...
	private Timer completionTimer(ProtocolKeyword protocolKeyword) {
		...
		if (enableHistogram) {
			timer.publishPercentileHistogram().minimumExpectedValue(MIN_LATENCY).maximumExpectedValue(MAX_LATENCY);
		}
